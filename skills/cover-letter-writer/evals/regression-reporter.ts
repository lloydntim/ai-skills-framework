...
function compatibilityLines(report: RegressionReport): string[] {
  const lines: string[] = [];
  lines.push(`Comparability: ${report.compatibility.level}${report.overridden ? ' (OVERRIDDEN by --allow-incompatible)' : ''}`);
...
export function formatRegressionReport(
  ...
  const lines: string[] = [];
  lines.push(`COVER LETTER WRITER REGRESSION REPORT — variant ${variant}`);
  lines.push('===============================================');
  lines.push('');
  ...
  if (report.deltaCasesCompared === 0) {
    ...
    lines.push(`Overall deltas (variant ${variant}, over ${report.deltaCasesCompared} comparable case(s)):`);
  ...
  if (tokens.casesCompared === 0) {
    ...
    const change =
      tokens.increasePercent === null ? 'n/a' : `${tokens.increasePercent >= 0 ? '+' : ''}${tokens.increasePercent.toFixed(1)}%`;
    const flag = tokens.exceeded ? '  <- beyond tokenIncreasePercent threshold' : '';
    lines.push(
      `Tokens over ${tokens.casesCompared} comparable case(s): ` +
  ...
  for (const c of report.caseComparisons) {
    ...
    lines.push(`  ${`[${c.status}]`.padEnd(10)} ${c.caseId}: overall ${formatScore(c.previous?.overall)} -> ${formatScore(c.current.overall)}${suffix}`);
  }
  lines.push('');
