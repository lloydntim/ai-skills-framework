...
describe('summarizePositionBias', () => {
  ...
  it('excludes and separately counts results that predate displayedWinner, rather than guessing', () => {
  ...
  it('never reconstructs the missing display mapping from the canonical winner', () => {
    // Even though `winner` + `variantA`/`variantB` would let you *guess* a display mapping, doing
    // so would conflate the canonical and display-slot alphabets. Confirm the guess is never made:
    // this record's canonical winner is 'A' but it must still be skipped, not double-counted as if
    // display slot A won.
    const noDisplayMapping = pairwise({ winner: 'A', variantA: 'A', variantB: 'B' });
    delete (noDisplayMapping as Partial<PairwiseResult>).displayedWinner;
    delete (noDisplayMapping as Partial<PairwiseResult>).displayedAsA;
    delete (noDisplayMapping as Partial<PairwiseResult>).displayedAsB;

    const summary = summarizePositionBias([noDisplayMapping]);

    expect(summary.skippedUnreconstructable).toBe(1);
    expect(summary.slotAWins).toBe(0);
    expect(summary.slotBWins).toBe(0);
  });
